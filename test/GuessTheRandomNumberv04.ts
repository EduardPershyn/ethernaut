...
describe(NAME, function () {
    ...
    async function setup() {
        ...
        deployBlockNumber = victimContract.deployTransaction.blockNumber;
        deployTime = (await ethers.provider.getBlock(deployBlockNumber)).timestamp;
        //console.log(deployBlockNumber, deployTime);
    ...
    describe("exploit", async function () {
        ...
        it("conduct your attack here", async function () {
            const AttackerFactory = await ethers.getContractFactory("Solution3v04");
            attackerContract = await AttackerFactory.connect(attackerWallet).deploy(victimContract.address, deployBlockNumber, deployTime);
